...
class SparseTranscoder(HookedRootModule):
    ...
    def load_from_hugging_face(
        ...
        file_template="final_sparse_autoencoder_gpt2-small_blocks.{}.ln2.hook_normalized_24576.pt",
    ):
        file_name = file_template.format(layer)
        file_path = hf_hub_download(repo_id, file_name)

        return cls.load_from_pretrained(file_path).train(False)
