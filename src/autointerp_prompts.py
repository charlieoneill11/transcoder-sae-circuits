...
You will be given a list of text examples on which the neuron activates. The specific tokens which cause the neuron to activate will appear between indicators will appear between {{tl}} and {{tr}} (like {{tl}}this{{tr}}). If a sequence of consecutive tokens all cause the neuron to activate, each token in the sequence will have a non-zero score, denote by being in between {{tl}} and {{tr}} like {{tl}}this{{tr}}.

Step 1: For each text example in turn, note which tokens (i.e., words, fragments of words, or symbols) caused the neuron to activate. The tokens which caused the neuron to activate highly will appear between {{tl}} and {{tr}} (like {{tl}}this{{tr}}). Then note which token came immediately before the activating token, for each activating token. 
Step 2: Look for patterns in the tokens you noted down in Step 1.
Step 3: Write down several general shared features of the text examples.
...
EXAMPLE_I = f"""
Example 1: Ralph and Kathy {{tl}}rightly{{tr}} state, that had
Example 2: Much love from the Cherubs! \n\n - Jason and {{tl}}Jordan{{tr}}! NEW UPDATE
Example 3: and Peggy {{tl}}hold{{tr}} Dodd {{tl}}hostage{{tr}} in a hunting cabin for two days, and in the end
...
EXAMPLE_1 = f"""
Example 1: and he was {{tl}}over the moon{{tr}} to find
Example 2: we'll be laughing {{tl}}till the cows come home{{tr}}! Pro
Example 3: thought Scotland was boring, but really there's more than {{tl}}meets the eye{{tr}}! I'd
...
EXAMPLE_2 = f"""
Example 1: a river is wide but the ocean is {{tl}}wider{{tr}}. The ocean
Example 2: every year you get {{tl}}taller{{tr}}, "
Example 3: the hole was {{tl}}smaller{{tr}} but {{tl}}deeper{{tr}} than the
...
EXAMPLE_3 = f"""
Example 1: something happening inside my {{tl}}house{{tr}}, he
Example 2: presumably was always contained in a {{tl}}box{{tr}}, " according
Example 3: people were coming into the {{tl}}smoking area{{tr}}" 
Example 4: Patrick: " why are you getting in the {{tl}}way{{tr}}? " Later,
