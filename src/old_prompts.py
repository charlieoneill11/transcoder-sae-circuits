...
You will be given a list of text examples on which the neuron activates. The specific tokens which cause the neuron to activate will appear between << and >>. If a sequence of consecutive tokens all cause the neuron to activate, each token in the sequence will have a non-zero score.
...
def get_opening_prompt(examples, top_logits):
    ...
    opening_prompt = AGENT_START.format(
        examples=examples_str, top_logits=top_logits_str
    )
