...
(we'll call this {{head_label}} from here on out).  We're specifically studying the following sentence:

[SEQUENCE_BEING_STUDIED]
{{ sequence }}
[\\SEQUENCE_BEING_STUDIED]

By examining {{head_label}}, we've found that {{head_label}} moves information from '{{source_token}}' to '{{dest_token}}'.
Specifically, here's the sentence we're studying with the following annotations: We've put the "source" token in delimiters 
{{sl}} and {{sr}}, and we've put the "destination" token in delimiters {{dl}} and {{dr}}:

[ANNOTATED_SEQUENCE]
{{source_dest_annotated_seq}}
[\\ANNOTATED_SEQUENCE]

From a previous analysis, we know that this attention head causes a particular neuron to fire strongly, and we know what this 
neuron represents. Here's that description:
...
Specifically, here's the sequence we're studying. We've put the "source" token in delimiters {{sl}} and {{sr}}, and we've put 
the "destination" token in delimiters {{dl}} and {{dr}}:
...
Step 3: Write down any similarities you see between the MAX_ACTIVATING_EXAMPLES and the SENTENCE_BEING_STUDIED.
Specifically pay attention similarities between the context tokens from MAX_ACTIVATING_EXAMPLES and SENTENCE_BEING_STUDIED.

Step 4: Write down any patterns you see in the TOP_PROMOTED_TOKENS.  If no patterns are apparent, just write "No 
apparent patterns -- Ignoring Top Promoted Tokens!" and ignore the top promoted tokens for the remainder of the analysis

Step 5: Write down several general shared features of the MAX_ACTIVATING_EXAMPLES, taking into account the 
TOP_PROMOTED_TOKENS if relevant.

Step 6: Based on the patterns you found in the MAX_ACTIVATING_EXAMPLES and the TOP_PROMOTED_TOKENS (if the 
...
Write your Step 6 explanation in the following form based on the neuron you're analyzing:
...
## STEP 2:
Now that you've analyzed the query, key, and value neurons that {{head_label}} uses, you should be able to figure out
how {{head_label}} computes the output neuron.  Perform the following tasks:

{# 1. Start by rewriting the head output neuron description and the annotated sentence being studied #}
...
2. Write out any similarities you see between the QUERY_NEURON_EXPLANATION and the KEY_NEURON_EXPLANATION.  What information
does {{head_label}} extract from information corresponding to the QUERY_NEURON_EXPLANATION that matches information
in KEY_NEURON_EXPLANATION?
{#2. Create a hypothesis for what computation {{head_label}} uses to produce query and key vectors that match,
and what information is transfered from the source token to the output based on the value vector of the source token #}
3. Based on the previous steps, give a description of what {{head_label}} is doing here. Specifically include how
{{head_label}} matches the query and key, and describe what information is tranfered from the source token in the value vector
...
Finally, based on your description of {{head_label}}, provide a short label for the head that distills your description
into a label. Provide your label in the following form:

[ATTENTION_HEAD_LABEL]
<your label>
[\\ATTENTION_HEAD_LABEL]
...
## In this prompt, we don't do feature interp on the fly
stripped_prompt = """
{{ basic_context }}

Right now, you're attempting to understand the computation performed by head {{head}} in layer {{layer}} of the transformer.
(we'll call this {{head_label}} from here on out).  From a previous analysis, we know that this attention head causes a
particular neuron to fire strongly, and we know what this neuron does. Here's that description:
...
In order to figure this out, you will analyze the inputs to {{head_label}} and deduce how the attention head uses that
information to produce the neuron corresponding to HEAD_OUTPUT_VECTOR_DESCRIPTION.

Now then, here's some useful background information:

The purpose of an attention head is to move information from a previous token to a later token within
a transformer.  Here's how this works: given a pair of tokens, the attention head computes a "query vector" for the later token,
and it computes a "key vector" and a "value vector" for the earlier token. If the query vector of the later token matches with
the key vector of the earlier token, the attention head moves the earlier token's value vector to the later token.
...
From a previous analysis, we've been able to isolate the particular neurons that the transformer uses to compute these
query, key, and value vectors.  Additionally,

Your job in this step is to figure out what these neurons represent, so we can
then figure out how {{head_label}} uses this information to produce the output neuron.
...
There are three inputs to an attention head: 1) the query vector 2) the key vector and 3) the value vector.
