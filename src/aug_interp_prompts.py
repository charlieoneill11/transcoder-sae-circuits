...
def clean_examples(examples: List[Tuple[str, str]]) -> List[Tuple[str, str]]:
    def clean_text(text: str) -> str:
        # Use regex to remove [[ and ]] while keeping the text inside intact
        cleaned_text = re.sub(r'\[\[|\]\]', '', text)
        return cleaned_text

    cleaned_examples = [(clean_text(base), clean_text(annotated)) for base, annotated in examples]
    return cleaned_examples

def main_aug_interp_prompt_v2(
    examples: List[str], token_lr=("<<", ">>")
):
    tl, tr = token_lr
...
INPUT_DESCRIPTION: 
You will be given several examples of text that activate the neuron.  First we'll provide the
example text without any annotations, and then we'll provide the same text with annotations that show the specific tokens that caused the neuron to activate.
...
Step 3: Based on the patterns you found between the activating token and the relevant context, write down your best explanation for what this neuron is responsible for.  Propose your explanation in the following form: 
[EXPLANATION]: <your explanation>
