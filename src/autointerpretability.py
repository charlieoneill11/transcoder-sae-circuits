...
class CircuitPrediction:
    ...
    def create_circuit_hypergraph(self):
        circuit_hypergraph = {
            label: {"freq": 0, "features": []} for label in self.component_labels
        }
        for layer, freq in enumerate(self.attn_freqs):
            for head, freq_head in enumerate(freq):
                ...
                circuit_hypergraph[label]["features"].extend(
                    self.features_for_heads[layer][head]
                )
    ...
    def visualize_circuit(self, circuit: np.ndarray, additional_title=""):
        ...
        fig = px.imshow(
            circuit_array,
            labels=dict(x="Attention Head", y="Layer"),
            width=500,
    ...
    def unique_feature_array(self, visualize: bool = False):
        ...
        for i, label in enumerate(self.component_labels):
            layer = i // 13
            head = i % 13
            unique_features_array[layer, head] = len(
                set(self.circuit_hypergraph[label]["features"])
            )
        if visualize:
            fig = px.imshow(
                unique_features_array,
                labels=dict(x="Attention Head", y="Layer"),
                width=450,
...
def get_top_k_activating_examples(feature_scores, tokens, model, k=5):
    ...
    top_k_batch_indices, top_k_seq_indices = np.unravel_index(
        top_k_indices, feature_scores.shape
    )
    ...
    top_k_scores_per_seq = [
        feature_scores[batch_idx].tolist() for batch_idx in top_k_batch_indices
    ]
    ...
    def strategy(cd: CircuitDiscovery):
        if pass_based:
            for _ in range(passes):
                cd.add_greedy_pass(
                    contributors_per_node=node_contributors, minimal=first_pass_minimal
                )
                if do_sub_pass:
